...
def init_system_config(admin):
    ...
    if SystemConfig.query.count() == 0:
        ...
        db.session.commit()
        print(f"✓ {len(configs_to_create)} 个系统配置项创建成功")
    else:
        print("✓ 系统配置已存在")
...
def main():
    ...
    try:
        with app.app_context():
            ...
            init_roles()
            init_organization()
            init_personal_kb(admin)
            init_doc_templates(admin)
            init_briefing_data()
            init_meeting_rooms(admin)
            init_archive_data()
            init_system_config(admin)
            perform_migrations()
