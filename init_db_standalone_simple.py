...
def init_all_tables():
    print_header("1. 创建数据库表")
    with app.app_context():
        db.create_all()
        print("OK: 所有表创建完成")

def init_admin_user():
    print_header("2. 初始化管理员用户")
    with app.app_context():
    ...
    with app.app_context():
        if Role.query.count() == 0:
            ...
            for role in default_roles:
                db.session.add(role)
            db.session.commit()
            print(f"OK: {len(default_roles)} 个角色创建成功")
        else:
            print("OK: 角色已存在")
    return True

def init_organization():
    print_header("4. 初始化组织架构")
    with app.app_context():
...
本纪要经与会人员审阅，如无异议，自发出之日起生效。"""
            ...
            for tmpl in default_templates:
                db.session.add(tmpl)
            db.session.commit()
            print(f"OK: {len(default_templates)} 个公文模板创建成功")
        else:
            print("OK: 公文模板已存在")
    return True

def init_briefing_data():
    print_header("7. 初始化简报系统数据")
    with app.app_context():
        if BriefingSource.query.count() == 0:
            ...
            for src in default_sources:
                db.session.add(src)
            db.session.commit()
            print(f"OK: {len(default_sources)} 个简报数据源创建成功")
        else:
            print("OK: 简报数据源已存在")
        ...
        if BriefingKeyword.query.count() == 0:
            ...
            for kw in default_keywords:
                db.session.add(kw)
            db.session.commit()
            print(f"OK: {len(default_keywords)} 个简报关键词创建成功")
        else:
            print("OK: 简报关键词已存在")
    return True

def init_meeting_rooms():
    print_header("8. 初始化会议室")
    with app.app_context():
        admin = User.query.filter_by(username='admin').first()
        if MeetingRoom.query.count() == 0:
            ...
            for room in default_rooms:
                db.session.add(room)
            db.session.commit()
            print(f"OK: {len(default_rooms)} 个会议室创建成功")
        else:
            print("OK: 会议室已存在")
    return True

def init_archive_data():
    print_header("9. 初始化档案系统数据")
    if not HAS_ARCHIVE:
        print("SKIP: 档案模型未找到，跳过初始化")
        return True
    with app.app_context():
...
def init_system_config():
    ...
    with app.app_context():
        ...
        if SystemConfig.query.count() == 0:
            ...
            db.session.commit()
            print(f"OK: {len(configs_to_create)} 个系统配置项创建成功")
        else:
            print("OK: 系统配置已存在")
    return True

def perform_migrations():
    print_header("11. 执行数据库迁移")
    with app.app_context():
        try:
            ...
            if 'crawler_tasks' in tables:
                ...
                if 'max_depth' not in columns:
                    ...
                    db.session.commit()
                    print("OK: 添加 crawler_tasks.max_depth 字段成功")
                else:
                    print("OK: crawler_tasks.max_depth 字段已存在")
            else:
                print("OK: crawler_tasks 表还未创建，无需迁移")
